-- Keep Guestbook submissions out of the challenge feeds explicitly.
--
-- Both functions already select on `metadata ? 'challenge_id'`, and the client
-- tags a submission as either a challenge entry or a Guestbook message but
-- never both, so today no Guestbook row can reach these results. That is an
-- accident of how the writer happens to behave, though, not a rule the reader
-- enforces: a row carrying both keys — a future feature, a repair script, a
-- hand-written insert — would surface every guest's private message to every
-- other guest through `get_guest_challenge_photos`.
--
-- The Guestbook's privacy rule is that only its own two accessors ever return
-- its rows, so state that here rather than relying on the writer's discipline.
...
comment on function public.get_host_challenge_photos(uuid) is
  'Returns all ready, undeleted challenge submissions for a host-managed celebration, including media and uploader metadata. Never returns Guestbook messages.';

revoke all on function public.get_host_challenge_photos(uuid) from public, anon;
grant execute on function public.get_host_challenge_photos(uuid) to authenticated;
...
comment on function public.get_guest_challenge_photos(text, text) is
  'Returns ready, undeleted challenge submissions for a guest-joined event, including media and ownership metadata. Never returns Guestbook messages.';

revoke all on function public.get_guest_challenge_photos(text, text) from public;
grant execute on function public.get_guest_challenge_photos(text, text) to anon, authenticated;
