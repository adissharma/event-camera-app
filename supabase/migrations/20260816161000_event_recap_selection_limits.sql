-- Keep on-demand recap requests bounded enough for predictable render cost and
-- pacing: at least three moments, up to 120 total items, and up to 20 videos.
...
begin
  ...
  if p_selected_media_ids is null or cardinality(p_selected_media_ids) < 3 then
    raise exception 'Choose at least 3 photos or videos for the recap.' using errcode = '22023';
  end if;
  ...
  if cardinality(v_selected) > 120 then
    raise exception 'Choose up to 120 items for one recap.' using errcode = '22023';
  end if;
  ...
  select
    count(*),
    count(*) filter (where mi.media_type = 'video')
  into v_valid_count, v_video_count
  ...
  if v_video_count > 20 then
    raise exception 'Choose up to 20 videos for one recap.' using errcode = '22023';
  end if;
