-- Fix get_guest_gallery RPC to resolve celebration by lower(event_code) instead of public_slug.
-- This was accidentally reverted in 20260809080000_guest_gallery_is_mine.sql.
...
comment on function public.get_guest_gallery is
  'Retrieves event details, session configurations, and guest-specific photo list securely validating the guest token. Includes is_mine flag so guests can identify and delete their own photos. Resolves the celebration by its guest-facing event_code.';
