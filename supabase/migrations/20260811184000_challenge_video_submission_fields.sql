-- Include video metadata in Challenge submission RPCs.
--
-- Challenge media now supports both photos and videos. The client needs the
-- media fields to render videos as videos inside the Challenge story instead
-- of treating every submission as an image.
...
comment on function public.get_host_challenge_photos(uuid) is
  'Returns all ready, undeleted challenge submissions for a host-managed celebration, including media and uploader metadata.';
...
comment on function public.get_guest_challenge_photos(text, text) is
  'Returns ready, undeleted challenge submissions for a guest-joined event, including media and ownership metadata.';
