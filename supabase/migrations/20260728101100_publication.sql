-- Publication and entitlement activation.
--
-- Publishing is the moment a draft becomes something guests can join. It must
-- be idempotent: a host on a bad connection who taps "Create my event" twice,
-- or whose request times out after the server committed, must end up with one
-- published event and one access link — not two.

-- ---------------------------------------------------------------------------
-- Entitlement activation
-- ---------------------------------------------------------------------------

create or replace function private.activate_plan_entitlements(
...
begin
  insert into public.celebration_entitlements (
    ...
    granted_by_plan_id, granted_by_purchase_id
  )
  select p_celebration_id, pe.entitlement_key, pe.value, p_plan_id, p_purchase_id
  from public.plan_entitlements pe
  where pe.plan_id = p_plan_id
  on conflict (celebration_id, entitlement_key) do update
    set value = excluded.value,
        granted_by_plan_id = excluded.granted_by_plan_id,
        granted_by_purchase_id = excluded.granted_by_purchase_id,
        granted_at = now();
end;
$$;

comment on function private.activate_plan_entitlements is
  'Copies a plan''s grants onto a celebration. Idempotent — re-running after a '
  'retried verification converges rather than duplicating.';

-- ---------------------------------------------------------------------------
-- Publish
-- ---------------------------------------------------------------------------

create type public.published_celebration as (
  celebration_id uuid,
...
begin
  ...
  -- Serialises concurrent publish attempts on the same row, which is what makes
  -- the double-tap case safe rather than merely unlikely.
  for update;
  ...
  if v_celebration.status = 'published' then
    return (
      ...
      v_celebration.published_at, true
  ...
  return (
    v_celebration.id, v_session.id, v_celebration.public_slug,
    v_celebration.published_at, false
  )::public.published_celebration;
end;
$$;
...
-- ---------------------------------------------------------------------------
-- Reading a guest link back
-- ---------------------------------------------------------------------------

create or replace function public.get_guest_access_link(p_event_session_id uuid)
returns table (access_link_id uuid, is_active boolean, expires_at timestamptz)
language sql
stable
security definer
set search_path = ''
as $$
  select al.id, al.is_active, al.expires_at
  from public.access_links al
  where al.event_session_id = p_event_session_id
    and al.kind = 'guest'
    and private.can_manage_event_session(p_event_session_id)
  order by al.created_at desc
  limit 1;
$$;

comment on function public.get_guest_access_link is
  'Metadata only. The token itself is stored as a digest and is unrecoverable — '
  'the plaintext is returned exactly once at creation. Regenerating the link is '
  'the recovery path if a host loses it.';

revoke all on function public.get_guest_access_link from public, anon;
grant execute on function public.get_guest_access_link to authenticated;
