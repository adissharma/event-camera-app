-- get_guest_gallery's `session` object never included `ends_at`, so every
-- guest-facing gallery read (the web gallery countdown among them) fell back
-- to `celebrations.ends_at` instead of the session's own end time — the
-- field the host's "manage event" end-date editor actually writes. A host
-- who extended their event's session end date kept seeing "Ended" on that
-- surface even though the session itself was still open.
...
comment on function public.get_guest_gallery(text, text) is
  'Returns a guest''s own gallery view (celebration + session + photos), including the session''s own ends_at so a host-extended event does not read as ended.';
...
-- One-time backfill: `celebrations.ends_at` is a denormalized copy of the
-- primary session's end time that the client now keeps in sync on every
-- edit (see the celebration-detail.ts fix), but events edited before that
-- fix shipped were left with a stale `celebrations.ends_at` from creation
-- time. Anything still reading that column directly (the fallback in
-- several client call sites) needs it to match the session it was copied
-- from.
update public.celebrations c
set ends_at = s.ends_at
from public.event_sessions s
where s.celebration_id = c.id
  and s.deleted_at is null
  and s.sequence_number = (
    select min(sequence_number)
    from public.event_sessions
    where celebration_id = c.id
      and deleted_at is null
  )
  and c.ends_at is distinct from s.ends_at;
