-- Fix get_guest_gallery RPC to use original_storage_path instead of storage_path.
-- This was accidentally reverted in 20260809080000_guest_gallery_is_mine.sql and missed in 20260809090000.
...
begin
  ...
  -- Resolve the celebration using event_code instead of public_slug
...
comment on function public.get_guest_gallery is
  'Retrieves event details, session configurations, and guest-specific photo list securely validating the guest token. Includes is_mine flag so guests can identify and delete their own photos. Resolves the celebration by its guest-facing event_code and references original_storage_path.';
