-- Event-level video capture plumbing.
--
-- The schema already supports `media_type = 'video'` and event sessions carry
-- `allowed_media_types`, but the active creation/upload RPCs still hard-coded
-- photo behavior. This migration wires the existing data model through the
-- current client-facing functions so video can be enabled per event while
-- remaining fully backward compatible for photo-only sessions.
...
create or replace function public.create_celebration_with_default_session(
  ...
  p_allowed_media_types public.media_type[] default array['photo']::public.media_type[],
...
declare
  ...
  v_allowed_media_types public.media_type[] := coalesce(
    p_allowed_media_types,
    array['photo']::public.media_type[]
  );
...
begin
  ...
  if not ('photo' = any (v_allowed_media_types)) then
    v_allowed_media_types := array_prepend('photo'::public.media_type, v_allowed_media_types);
  end if;
...
begin
  ...
  if p_media_type = 'audio' then
    raise exception 'audio uploads are not supported by this endpoint yet' using errcode = '42501';
  end if;
...
begin
  ...
  if v_item.media_type = 'video' then
    if coalesce(p_duration_ms, 0) <= 0 or p_duration_ms > 30000 then
      raise exception 'video must be 30 seconds or less' using errcode = '22023';
    end if;
  end if;
  ...
  update public.media_items
    set status = 'ready',
        ...
        width = p_width,
        height = p_height,
        duration_ms = case when v_item.media_type = 'video' then p_duration_ms else null end,
