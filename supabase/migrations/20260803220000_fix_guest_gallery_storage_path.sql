-- Fixes a column-name bug in get_guest_gallery: media_items has no
-- `storage_path` column — the app-facing photo path is
-- `original_storage_path` (see 20260728100300_media.sql). The RPC has raised
-- `42703 column mi.storage_path does not exist` on every call since it was
-- written, which meant a guest could never load their event page or gallery.
--
-- create or replace function with an unchanged signature updates the
-- function in place; no drop is needed and existing grants are preserved.
