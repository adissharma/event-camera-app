-- Restore guest ownership metadata for regular gallery media.
--
-- The video/Guestbook RPC replacements kept the privacy filtering correct, but
-- dropped the ownership fields that the app uses to show "Delete Photo/Video"
-- to the guest who posted the item.
...
comment on function public.get_guest_gallery(text, text) is
  'Retrieves guest-visible gallery details, excluding private Guestbook media and including ownership metadata for guest deletes.';
