-- Make host-only galleries truly host-only for guests.
--
-- The client now supports a guest reveal choice of "Never", which maps to
-- `gallery_visibility = 'hosts_only'`. Guests should not receive even their
-- own photos in that state; the gallery is for hosts only.
