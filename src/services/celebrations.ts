...
export async function listCelebrations(): Promise<CelebrationSummary[]> {
  try {
    ...
    const { data, error } = await client
      .from('celebrations')
      .select(
        `id, title, status, cover_storage_path, public_slug, starts_at, ends_at, timezone, default_theme_id,
      ...
      return {
        ...
        primarySession: primary
          ? {
              ...
              shot_limit_per_guest: primary.shot_limit_per_guest,
            }
          : null,
      };
    });
  } catch (e) {
    console.warn('Failed to fetch celebrations from Supabase, loading from AsyncStorage:', e);
