/*
 * Stands in for `@shopify/react-native-skia` so the real filter modules can run
 * under Node against `canvaskit-wasm` — the same Skia, compiled to WASM.
 *
 * The point of the lab is to exercise the shipped code, not a copy of it, so
 * anything reimplemented here is somewhere the preview could lie. Keep this to
 * pure API adaptation: no filter logic belongs in this file.
 */
let CK = null;

exports.__setCanvasKit = (canvasKit) => {
  CK = canvasKit;
...
exports.BlendMode = new Proxy({}, { get: (_, key) => CK.BlendMode[key] });
exports.TileMode = new Proxy({}, { get: (_, key) => CK.TileMode[key] });
exports.FilterMode = new Proxy({}, { get: (_, key) => CK.FilterMode[key] });
exports.MipmapMode = new Proxy({}, { get: (_, key) => CK.MipmapMode[key] });

/*
 * CanvasKit takes a bare nine-element array as a local matrix; React Native
 * Skia wraps one in an object with chainable mutators. Be both: an array
 * CanvasKit accepts, carrying the mutators the shipped code calls. Same
 * pre-concat order as SkMatrix, so translate-then-scale composes to T*S.
 */
function makeMatrix() {
